...
class TestSubtitleCleaner:
    ...
    def test_remove_parentheses_content(self):
        assert TextCleaner._remove_parentheses_content("(Hello) World") == " World"
        assert TextCleaner._remove_parentheses_content("( Hello ) World") == " World"
        assert (
            TextCleaner._remove_parentheses_content("Hello (whispering) world")
            == "Hello  world"
        )
        assert (
            TextCleaner._remove_parentheses_content("Hello ( whispering ) world")
        ...
        assert TextCleaner._remove_parentheses_content("Hello (World)") == "Hello "
        assert TextCleaner._remove_parentheses_content("Hello ( World )") == "Hello "
    ...
    def test_remove_bracket_content(self):
        ...
        assert TextCleaner._remove_bracket_content("[ Hello ] World") == " World"
        assert (
            TextCleaner._remove_bracket_content("Hello [whispering] world")
            == "Hello  world"
        )
        assert (
            TextCleaner._remove_bracket_content("Hello [ whispering ] world")
    ...
    def test_remove_curly_bracket_content(self):
        assert TextCleaner._remove_curly_bracket_content("{Hello} World") == " World"
        assert TextCleaner._remove_curly_bracket_content("{ Hello } World") == " World"
        assert (
            TextCleaner._remove_curly_bracket_content("Hello {whispering} world")
            == "Hello  world"
        )
        assert (
            TextCleaner._remove_curly_bracket_content("Hello { whispering } world")
        ...
        assert TextCleaner._remove_curly_bracket_content("Hello {World}") == "Hello "
        assert TextCleaner._remove_curly_bracket_content("Hello { World }") == "Hello "
        assert TextCleaner._remove_curly_bracket_content("{ }") == ""
        assert TextCleaner._remove_curly_bracket_content("{}") == ""

    def test_remove_hash_symbol_content(self):
        assert TextCleaner._remove_hash_symbol_content("#Hello# World") == " World"
        assert TextCleaner._remove_hash_symbol_content("# Hello # World") == " World"
        assert (
            TextCleaner._remove_hash_symbol_content("Hello #whispering# world")
            == "Hello  world"
        )
        assert (
            TextCleaner._remove_hash_symbol_content("Hello # whispering # world")
        ...
        assert TextCleaner._remove_hash_symbol_content("Hello #World#") == "Hello "
        assert TextCleaner._remove_hash_symbol_content("Hello # World #") == "Hello "
    ...
    def test_remove_music_indication(self):
        assert (
            TextCleaner._remove_music_indication("♪ Hello ♪ world") == " Hello  world"
        )
        assert TextCleaner._remove_music_indication("♪Hello♪ world") == "Hello world"
        assert (
            TextCleaner._remove_music_indication("Hello ♪ singing ♪ world")
            == "Hello  singing  world"
        )
        assert (
            TextCleaner._remove_music_indication("Hello ♪singing♪ world")
    ...
    def test_replace_double_hyphens(self):
        assert TextCleaner._fix_double_hyphens("It wasn't--") == "It wasn't\u2014"
        assert TextCleaner._fix_double_hyphens("It was not--") == "It was not\u2014"
        assert TextCleaner._fix_double_hyphens("Wait-- no--") == "Wait\u2014 no\u2014"
        assert TextCleaner._fix_double_hyphens("Hello world") == "Hello world"
        assert TextCleaner._fix_double_hyphens("single-hyphen") == "single-hyphen"
        assert (
            TextCleaner._fix_double_hyphens(
    ...
    def test_remove_speaker_identification(self):
        ...
        assert (
            TextCleaner._remove_speaker_identification("MRS. WOLOWITZ: How are you?")
            == " How are you?"
        )
        assert (
            TextCleaner._remove_speaker_identification("O'BRIEN: How are you?")
            == " How are you?"
        )
        assert (
            TextCleaner._remove_speaker_identification("MARY-ANN: How are you?")
            == " How are you?"
        )
        assert (
            TextCleaner._remove_speaker_identification("Person: How are you?")
    ...
    def test_final_cleanup_multiple_spaces(self):
        ...
        assert (
            TextCleaner._final_cleanup("Hello   there   world") == "Hello there world"
        )
        assert (
            TextCleaner._final_cleanup("  Leading  and  trailing  ")
            == "Leading and trailing"
        )

    def test_final_cleanup_spaces_before_punctuation(self):
        assert TextCleaner._final_cleanup("Hello , world !") == "Hello, world!"
        assert TextCleaner._final_cleanup("Hello . Goodbye !") == "Hello. Goodbye!"
        assert (
            TextCleaner._final_cleanup("Question ? Answer : Yes")
            == "Question? Answer: Yes"
        )
        assert (
            TextCleaner._final_cleanup("Hello , world ! How are you ?")
    ...
    def test_final_cleanup_multiline_whitespace(self):
        ...
        assert (
            TextCleaner._final_cleanup("   \n  Line 1  \n   \n  Line 2  \n   ")
            == "Line 1\nLine 2"
        )
        assert (
            TextCleaner._final_cleanup("Line 1   \n   \n   Line 2") == "Line 1\nLine 2"
        )
    ...
    def test_clean_subtitle_no_dashes_needed_in_mixed_patterns(self):
        subtitle = Subtitle(
            ...
            lines=["{excitedly} I have a theory!", "#Sighs# ♪ Not again... ♪"],
        )
